...
plan.remote('apt#setup', function (remote) {
	...
	remote.sudo('apt-get install htop');
	remote.sudo('apt-get -y install libvips libvips-dev');
	remote.sudo('apt-get -y install upstart');
	remote.sudo('mkdir -p /opt/upstart');
});

/**
 * ----------------------------------------------------------------
 * Upstart Setup (back to upstart, ubuntu 16.04)
 * ----------------------------------------------------------------
 */

plan.remote('upstart#setup', function (remote) {
	remote.sudo('apt-get -y install upstart-sysv');
	remote.sudo('update-initramfs -u');
	// then reboot server
});
...
plan.remote('nginx#setup', function (remote) {
...
plan.remote('nvm#setup', function (remote) {
	remote.exec('curl https://raw.githubusercontent.com/creationix/nvm/v0.32.0/install.sh | bash');
	...
	remote.with('cd /'+basedir, function () {
		...
		remote.with('cd '+appdir, function () {
			...
			remote.with('cd '+repodir, function () {
				remote.exec('~/.nvm/'+nvmInstall+'v'+conf.node+'/bin/npm install --verbose');
			});
		});
	});
});
...
plan.remote('app#deploy', function (remote) {
	remote.with('cd '+fulldir, function () {
		...
		remote.exec('~/.nvm/'+nvmInstall+'v'+conf.node+'/bin/npm install --verbose'); // install new npm packages
		remote.exec('~/.nvm/'+nvmInstall+'v'+conf.node+'/bin/pm2 gracefulReload all');
	});
});

plan.remote('app#resizer', function (remote) {
	remote.with('cd '+fulldir+'node_modules/app.io/', function () {
		remote.exec('~/.nvm/'+nvmInstall+'v'+conf.node+'/bin/npm install image-resizer-tmp');
	});
});
...
plan.remote('mongodb3.0#setup', function (remote) {
	remote.sudo('apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv 7F0CEB10');
...
plan.remote('mongodb3.2#setup', function (remote) {
	remote.sudo('sudo apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv EA312927');
	remote.exec("echo 'deb http://repo.mongodb.org/apt/ubuntu '$(lsb_release -sc)'/mongodb-org/3.2 multiverse' | sudo tee /etc/apt/sources.list.d/mongodb-org-3.2.list");
	remote.sudo('apt-get update');
	remote.sudo('apt-get install -y mongodb-org');
});
...
plan.local('upstart#load', function (local) {
	local.log('copy upstart files to remote host /opt/upstart folder');
	var target = plan.runtime.target;

	local.with('cd upstart/generated/'+app+'/'+target, function () {
		var files = local.ls();
		// rsync files to all the target's remote hosts
		local.transfer(files, '/opt/upstart');
	});
});
...
plan.remote('upstart#load', function (remote) {
	...
	remote.sudo('mkdir -p /opt/upstart');
	remote.with('cd /opt/upstart', function () {
...
plan.remote('couchdb#setup', function (remote) {
	remote.sudo('apt-get update');
	remote.sudo('apt-get -y install software-properties-common');
	remote.sudo('add-apt-repository -y ppa:couchdb/stable');
	remote.sudo('apt-get update');
	remote.sudo('apt-get -y install couchdb');
	remote.sudo('chown -R couchdb:couchdb /usr/bin/couchdb /etc/couchdb /usr/share/couchdb');
	remote.sudo('chmod -R 0770 /usr/bin/couchdb /etc/couchdb /usr/share/couchdb');
	remote.sudo('restart couchdb');
});
...
plan.remote('pm2#setup', function (remote) {
	remote.exec('~/.nvm/'+nvmInstall+'v'+conf.node+'/bin/npm install pm2 -g --verbose');
});

plan.remote('pm2#start', function (remote) {
	var cmd = 'pm2 start process.json';
	if(process.env.NODE_ENV)
		cmd += ' --env '+process.env.NODE_ENV;

	remote.with('cd '+fulldir, function () {
		remote.exec('~/.nvm/'+nvmInstall+'v'+conf.node+'/bin/'+cmd);
	});
});
